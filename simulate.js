...
// A page error is a FAILURE, not a log line.
//
// This printed "PAGE ERROR: ReferenceError: watch is not defined" and then
// exited 0, so a blank cashier screen shipped twice while the suite said
// it was fine. A test that reports a fault and passes anyway is worse than
// no test: it converts a visible bug into a trusted green run.
const pageErrors = [];
window.console.error = (...a) => {
  const text = a.map(x => (x && x.stack) ? x.stack.split('\n').slice(0, 3).join(' | ') : String(x)).join(' ');
  pageErrors.push(text);
  console.log('PAGE ERROR:', text);
};
window.addEventListener('error', (e) => {
  pageErrors.push(String(e.error || e.message));
  console.log('PAGE ERROR:', String(e.error || e.message));
});
window.addEventListener('unhandledrejection', (e) => {
  pageErrors.push(String(e.reason));
  console.log('PAGE REJECTION:', String(e.reason));
});

// Anything the page reports as an error fails the run, except the few
// benign notices jsdom itself emits. The list is deliberately tiny —
// every addition to it is a place the suite stops watching.
const IGNORABLE = [
  /Not implemented: HTMLCanvasElement/i,
  /Could not parse CSS stylesheet/i,
  /Error: Not implemented: window\.scrollTo/i,
];
function assertNoPageErrors(stage) {
  const real = pageErrors.filter(e => !IGNORABLE.some(rx => rx.test(e)));
  if (real.length) {
    console.log(`\nFAILED at ${stage}: the page raised ${real.length} error(s):`);
...
console.log('BOOT OK: .cashier-shell mounted');
  ...
  // Errors raised after boot — during cart, payment, dialogs — count too.
  assertNoPageErrors('end of run');
  console.log('\nboot simulation: no page errors');
  process.exit(0);
