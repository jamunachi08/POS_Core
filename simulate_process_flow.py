...
# Singles, taken from the real doctype JSON so the sim cannot drift.
import json as _json
_SINGLES = set()
_dtroot = os.path.join("alphax_pos_suite", "alphax_pos_suite", "doctype")
for _d in os.listdir(_dtroot):
    _f = os.path.join(_dtroot, _d, _d + ".json")
    if os.path.isfile(_f):
        _j = _json.load(open(_f, encoding="utf-8"))
        if _j.get("issingle"):
            _SINGLES.add(_j["name"])

frappe.get_meta = lambda dt: types.SimpleNamespace(issingle=dt in _SINGLES)
...
# Guard 8: a Single never routes to a list, never offers New, never counts.
# Opening "AlphaX POS Settings" as List raised TableMissingError on a live site.
_ROLES[:] = ["System Manager"]
d = api.get_flow()
singles_seen = 0
for st in d.get("stages", []):
    for n in st["nodes"]:
        if n.get("document_type") in _SINGLES:
            singles_seen += 1
            if not n.get("is_single"):
                print(f"   !! single not flagged: {n['document_type']}"); fail += 1
            if n.get("can_create"):
                print(f"   !! New offered on single: {n['document_type']}"); fail += 1
            if n.get("count") is not None:
                print(f"   !! count attempted on single: {n['document_type']}"); fail += 1
            if "/view/" in (n.get("route") or ""):
                print(f"   !! single routed to a view: {n['route']}"); fail += 1
if not singles_seen:
    print("   !! no single node exercised"); fail += 1
print(f"Singles checked  : {singles_seen} ({', '.join(sorted(_SINGLES & _EXISTING_DT))})")
