...
def get_readiness() -> dict:
    ...
    # ---- catalogue: what the till sells, and at what price ------------------
    # Walks the same chain the register walks (catalog.api.catalog_diagnose),
    # so this line and the cashier grid can never disagree.
    if terms:
        try:
            from alphax_pos_suite.alphax_pos_suite.catalog.api import catalog_diagnose
            diag = catalog_diagnose(terms[0].name)
        except Exception:
            diag = {}
        if diag.get("outlet"):
            unscoped = any(c["check"] == "catalogue is scoped" and not c["ok"]
                           for c in diag.get("checks", []))
            src = diag.get("source")
            site.append(_item(
                "catalogue", "What the till sells", "ما يبيعه الصندوق",
                "missing" if unscoped else "ok", "manual",
                count=diag.get("item_count"),
                detail=("No menu on the outlet and no domain item group, so the "
                        "till lists every sellable item on the site — including "
                        "items created by other apps. Assign a menu to the outlet."
                        if unscoped else
                        f"From {'menus' if src == 'menu' else 'the domain item group'}."),
                route="/app/alphax-pos-menu",
            ))
            chain = diag.get("price_chain") or []
            unpriced = diag.get("unpriced") or 0
            site.append(_item(
                "pricing", "Selling prices", "أسعار البيع",
                "ok" if (chain and not unpriced) else "missing", "manual",
                count=unpriced,
                detail=(" → ".join(f"{c['price_list']} ({c['source']})" for c in chain)
                        + (f". {unpriced} item(s) have no price in any of these and "
                           "sell at 0.00 — add Item Prices." if unpriced else "")
                        if chain else
                        "No price list anywhere: set the outlet's Default Price List."),
                route="/app/item-price",
            ))
...
# --------------------------------------------------------------------------
# network discovery (v15.17.0)
# --------------------------------------------------------------------------
#
# The scan itself runs in the AlphaX Bridge on a shop PC, because that is
# the only place a private address is reachable from. The browser asks the
# bridge on localhost to scan, then hands the result to the functions below.
# The server never probes an IP itself — from Frappe Cloud it could not.

_MAX_DEVICES = 256


def _clean_scan(result) -> dict:
    res = frappe.parse_json(result) if isinstance(result, str) else (result or {})
    if not isinstance(res, dict):
        frappe.throw(_("Invalid scan result"))
    devices = []
    for d in (res.get("devices") or [])[:_MAX_DEVICES]:
        if not isinstance(d, dict) or not d.get("ip"):
            continue
        devices.append({
            k: d.get(k) for k in (
                "ip", "kind", "label", "model", "vendor", "ports", "protocols",
                "print_port", "suggested_profile", "hostname", "bridge_version",
                "sys_name", "dns_name", "mac_address", "is_self", "snmp")
            if d.get(k) not in (None, "", [])
        })
    return {
        "scope": str(res.get("scope") or "")[:120],
        "from_ip": str(res.get("from_ip") or "")[:64],
...
def _match_terminals(devices: list) -> None:
    """Tag each device with the terminal it belongs to, where we can tell.

    Peer bridges match on hostname, which their /hello reports. A printer
    matches a Print Station whose bridge device already points at its IP.
    """
    if not devices:
        return
    by_host = {}
    for t in _terminals():
        host = (t.get("pc_hostname") or "").strip().lower()
        if host and not _UA_OS_TOKEN.match(host):
            by_host[host] = t.name
    for d in devices:
        if d.get("kind") == "alphax_bridge":
            h = (d.get("hostname") or "").strip().lower()
            d["terminal"] = by_host.get(h) or by_host.get(h.split(".")[0])


@frappe.whitelist()
def report_network_scan(terminal: str, result=None) -> dict:
    """Store what the bridge on ``terminal`` found on its network."""
    _require(READ_ROLES)
    if not frappe.db.exists("AlphaX POS Terminal", terminal):
        frappe.throw(_("Unknown terminal {0}").format(terminal))

    scan = _clean_scan(result)
    _match_terminals(scan["devices"])

    values = {}
    if _has("AlphaX POS Terminal", "bridge_network_json"):
        values = {
            "bridge_network_json": json.dumps(scan, indent=1)[:60000],
            "bridge_network_scanned_on": frappe.utils.now_datetime(),
            "bridge_network_scope": scan["scope"],
            "bridge_network_found": len(scan["devices"]),
        }
        frappe.db.set_value("AlphaX POS Terminal", terminal, values, update_modified=False)
        frappe.db.commit()

    printers = [d for d in scan["devices"] if d.get("kind") == "printer"]
    peers = [d for d in scan["devices"] if d.get("kind") == "alphax_bridge"]
    return {
        "ok": True,
        "stored": bool(values),
        "scope": scan["scope"],
        "printers": printers,
        "bridges": peers,
        "others": [d for d in scan["devices"] if d.get("kind") not in ("printer", "alphax_bridge")],
        "unmatched_bridges": [p["ip"] for p in peers if not p.get("terminal")],
    }


@frappe.whitelist()
def create_print_station_from_scan(terminal: str, device_name: str, ip: str,
                                   port: int = 9100, station_type: str = "Printer") -> dict:
    """Server half of 'use this printer'.

    The browser has already asked the bridge to register ``device_name``
    (POST /devices/network). This creates the matching Print Station whose
    ``bridge_target`` is that device name, which is what kot_api routes by.
    """
    _require(RUN_ROLES)
    ...
    try:
        ...
        frappe.throw(_("{0} is not an IP address").format(ip))
    if not (addr.is_private or addr.is_link_local or addr.is_loopback):
        frappe.throw(_("Only private network printers can be added."))
    if station_type not in ("Printer", "Kitchen Display"):
        station_type = "Printer"

    outlet = frappe.db.get_value("AlphaX POS Terminal", terminal, "pos_outlet")
    name = f"{device_name} - {outlet}"[:140] if outlet else device_name[:140]

    if frappe.db.exists("AlphaX POS Print Station", name):
        doc = frappe.get_doc("AlphaX POS Print Station", name)
        doc.bridge_target = device_name
        doc.enabled = 1
        doc.save(ignore_permissions=True)
        created = False
    else:
        has_default = outlet and frappe.db.exists(
            "AlphaX POS Print Station", {"outlet": outlet, "is_default": 1})
        doc = frappe.get_doc({
            "doctype": "AlphaX POS Print Station",
            "station_name": name,
            "outlet": outlet,
            "station_type": station_type,
            "bridge_target": device_name,
            "is_default": 0 if has_default else 1,
            "enabled": 1,
        }).insert(ignore_permissions=True)
        created = True
    ...
    frappe.db.commit()
    return {"ok": True, "station": doc.name, "created": created,
            "bridge_target": device_name, "address": f"{ip}:{int(port)}"}
