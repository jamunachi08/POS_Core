...
def update_table_status(table, status, party_size=None):
    ...
    # Occupancy and readiness are separate axes (v15.13). A party leaving
    # frees the table for the ledger but not for the next guest — it still
    # has to be bussed. Collapsing both facts into one field is why a
    # cleared-but-unwiped table looks bookable to the host.
    now = now_datetime()
    if doc.meta.has_field("readiness"):
        if status == "Occupied" and old_status != "Occupied":
            doc.last_seated_on = now
        if old_status == "Occupied" and status in ("Free", "Dirty"):
            doc.last_cleared_on = now
            # Leaving a table sets it dirty, always. Somebody has to say it
            # is clean; nothing should assume it.
            doc.readiness = "Needs Cleaning"
            doc.readiness_changed_on = now
            status = "Free"
            doc.status = "Free"
        if status == "Disabled":
            doc.readiness = "Out of Service"
            doc.readiness_changed_on = now
...
@frappe.whitelist()
def set_table_readiness(table: str, readiness: str):
    """Mark a table clean, dirty, or out of service.

    Separate endpoint on purpose: bussing is a different action performed
    by a different person at a different moment from seating, and the
    permission to do one should not imply the other.
    """
    valid = {"Ready", "Needs Cleaning", "Out of Service"}
    if readiness not in valid:
        frappe.throw(_("Invalid readiness: {0}").format(readiness))
    if not frappe.db.exists("AlphaX POS Table", table):
        frappe.throw(_("Table not found."))

    doc = frappe.get_doc("AlphaX POS Table", table)
    if not doc.meta.has_field("readiness"):
        return {"ok": False, "reason": "field_missing"}

    if readiness == "Ready" and doc.status == "Occupied":
        frappe.throw(_("{0} is still occupied.").format(table))

    doc.readiness = readiness
    doc.readiness_changed_on = now_datetime()
    if readiness == "Out of Service":
        doc.status = "Disabled"
    elif doc.status == "Disabled":
        doc.status = "Free"
    doc.save()

    frappe.publish_realtime(
        event="alphax_pos_table_status",
        message={"table": table, "floor": doc.floor,
                 "to_status": doc.status, "readiness": readiness},
    )
    return {"ok": True, "table": table, "status": doc.status, "readiness": readiness}
