import frappe
from alphax_pos_suite.alphax_pos_suite.reporting.order_totals import order_grand_total
...
def build_shift_close_context(shift_doc):
    ...
    if source == "Sales Invoice":
        # Sales Invoice HAS grand_total — accumulate from the list rows,
        # then load docs only for the payment breakdown.
        for o in orders:
        ...
        # AlphaX POS Order has no grand_total column; derive per order
        # from the linked invoice or the payment rows (order_totals.py).
        for o in orders:
            try:
                doc = frappe.get_doc("AlphaX POS Order", o["name"])
            except Exception:
                continue
            gt = order_grand_total(doc)
            if cint(o.get("is_return") or 0) == 1:
                return_value += gt
            else:
                sales_value += gt
...
def recompute_day_close(day_close_doc):
    ...
    if source == "Sales Invoice":
        ...
        orders = frappe.get_all(
            "AlphaX POS Order",
            filters=filters,
            fields=["name", "is_return", "discount_amount", "sales_invoice"],
        )
    ...
    if source == "Sales Invoice":
        ...
        # See order_totals.py — the order header has no grand_total.
        for o in orders:
            try:
                doc = frappe.get_doc("AlphaX POS Order", o["name"])
            except Exception:
