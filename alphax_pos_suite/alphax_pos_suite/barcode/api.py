...
def _resolve_price_list(pos_profile: str | None, outlet: str | None,
                        terminal: str | None = None) -> dict:
    """The pricing context for a scan. Same chain as the tapped grid.

    Returns a context rather than a single price list, because the answer
    depends on the item: the first list in the chain that actually prices
    it wins, exactly as it does on the grid.
    """
    return {"pos_profile": pos_profile, "outlet": outlet, "terminal": terminal}


def _item_price(item_code: str, price_list, standard_rate: float) -> float:
    """Rate on the shared catalogue chain; standard rate if nothing prices it."""
    from alphax_pos_suite.alphax_pos_suite.catalog.api import rate_for

    if isinstance(price_list, dict):
        rate, _pl, _src = rate_for(item_code, outlet=price_list.get("outlet"),
                                   terminal=price_list.get("terminal"),
                                   pos_profile=price_list.get("pos_profile"),
                                   standard_rate=flt(standard_rate))
        return flt(rate)

    # Legacy callers passing a bare price-list name keep their old behaviour.
    if price_list:
...
@frappe.whitelist()
def scan_barcode(code: str, pos_profile: str | None = None, outlet: str | None = None,
                 terminal: str | None = None) -> dict:
    ...
    price_list = _resolve_price_list(pos_profile, outlet, terminal)
