...
frappe.pages['alphax-pos-setup-wizard'].on_page_load = function(wrapper) {
  ...
  $(wrapper).on('click', '[data-action="next"]', async function(){
    ...
    if (state.step < 5){
      state.step++;
      render_step();
      // v15.16.0 — removed the user-agent "hostname" fallback. It copied the
      // first token of navigator.userAgent, which is the OS ("Windows NT
      // 10.0"), not the machine name, and that value then stuck on the
      // terminal because the bind step keeps an existing hostname. A browser
      // cannot read the PC name at all; the AlphaX Bridge reports the real
      // one on its first heartbeat. Leaving the field blank is correct.
      return;
    }
    // Step 5 → finish setup
    await finish_setup();
  ...
  async function finish_setup(){
    ...
    try {
      ...
      if (m.ok){
        ...
        // straight into it.
        try {
          localStorage.setItem('alphax_pos_terminal', m.terminal);
        } catch(e){}
        frappe.show_alert({
        ...
        setTimeout(() => {
          frappe.set_route('alphax-cashier');
        }, 1200);
      } else {
        frappe.msgprint({
