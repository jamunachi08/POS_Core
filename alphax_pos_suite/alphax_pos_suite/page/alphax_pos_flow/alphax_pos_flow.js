...
frappe.pages["alphax-pos-flow"].on_page_load = function (wrapper) {
	...
	function showSetupDialog(rd) {
		...
		d.fields_dict.body.$wrapper.html(`
			<div class="axf-setup">
				...
				<h5>${__("Site")}</h5>
				${siteRows}
				<h5 style="display:flex;align-items:center;gap:10px">${__("Terminals")}
					<a class="axf-btn axf-btn-new" data-scan="1" style="margin-inline-start:auto;text-transform:none;letter-spacing:0">${__("Scan network from this PC")}</a></h5>
		...
		$w.find("[data-scan]").on("click", () => { d.hide(); startScan(rd); });
	...
	// ------------------------------------------------------- network scan
	// The scan runs in the AlphaX Bridge on THIS PC, reached on localhost.
	// It cannot run on the server (Frappe Cloud cannot reach 192.168.x.x),
	// and the browser cannot port-scan. Only a bridge inside the shop can.
	const LS = { url: "alphax_bridge_url", token: "alphax_bridge_token", term: "alphax_pos_terminal" };
	const ls = (k) => { try { return localStorage.getItem(k) || ""; } catch (e) { return ""; } };

	async function bridgeFetch(path, opts = {}) {
		const base = (ls(LS.url) || "http://localhost:8420").replace(/\/+$/, "");
		...
		try {
			...
			return { status: 0, data: { error: e.name === "AbortError" ? "timeout" : String(e) } };
		} finally {
			clearTimeout(timer);
		}
	}

	function startScan(rd) {
		const terms = (rd.terminals || []).map((t) => t.terminal);
		const here = ls(LS.term);
		const d = new frappe.ui.Dialog({
			title: __("Scan network from this PC"),
			fields: [
				{ fieldtype: "HTML", fieldname: "note", options:
					`<p class="axf-note">${__("Runs in the AlphaX Bridge installed on the PC you are using now. Enter a printer's IP, a range like 192.168.1.0/24, or leave blank to scan this PC's own subnet. Private addresses only, at most 256 at a time.")}</p>` },
				{ fieldtype: "Data", fieldname: "target", label: __("IP address or range"),
				  description: __("e.g. 192.168.1.60  or  192.168.1.0/24  (blank = this PC's subnet)") },
				{ fieldtype: "Select", fieldname: "terminal", label: __("Record results against"),
				  options: terms.join("\n"), default: terms.includes(here) ? here : terms[0], reqd: 1,
				  description: here && terms.includes(here) ? __("This PC is bound to {0}.", [here]) : "" },
			],
			primary_action_label: __("Scan"),
			primary_action: async (v) => {
				d.hide();
				frappe.dom.freeze(__("Scanning the network… up to a minute for a full range"));
				const q = encodeURIComponent((v.target || "").trim());
				const r = await bridgeFetch(`/discover/network?target=${q}`, { timeout: 90000 });
				frappe.dom.unfreeze();
				if (r.status === 0) {
					return frappe.msgprint({ title: __("Bridge not reachable"), indicator: "red", message:
						__("No AlphaX Bridge answered on this PC. Network scanning runs from the bridge, so open this page on a PC where it is installed — usually the till itself.") });
				}
				if (r.status === 401) {
					return frappe.msgprint({ title: __("Bridge refused the request"), indicator: "orange", message:
						__("The bridge needs its pairing token. Open the cashier screen once on this PC so it pairs, then scan again.") });
				}
				if (r.status === 404) {
					return frappe.msgprint({ title: __("Bridge too old"), indicator: "orange", message:
						__("This bridge predates network discovery. Download and run the latest installer for this PC (v15.6.0 or newer).") });
				}
				if (r.status !== 200) {
					return frappe.msgprint({ title: __("Scan refused"), indicator: "orange",
						message: frappe.utils.escape_html(r.data.error || ("HTTP " + r.status)) });
				}
				frappe.call({
					method: `${SETUP}.report_network_scan`,
					args: { terminal: v.terminal, result: JSON.stringify(r.data) },
					callback: (res) => showScanResult(v.terminal, r.data, res.message || {}),
				});
			},
		});
		d.show();
	...
	function showScanResult(terminal, raw, rep) {
		const esc = frappe.utils.escape_html;
		const row = (x, act) => `
			<div class="axf-row">
				<span class="axf-s ${x.kind === "printer" ? "axf-s-ok" : x.kind === "alphax_bridge" ? "axf-s-dev" : "axf-s-skip"}">
					${x.kind === "printer" ? "⎙" : x.kind === "alphax_bridge" ? "⌁" : "•"}</span>
				<div class="axf-row-main">
					<div class="axf-row-title">${esc(x.label || x.ip)}</div>
					<div class="axf-row-detail">${[x.ip, x.mac_address, x.dns_name || x.sys_name,
						(x.protocols || []).join("/"), x.terminal ? __("terminal {0}", [x.terminal]) : ""]
						.filter(Boolean).map(esc).join(" · ")}</div>
				</div>
				<div class="axf-row-act">${act || ""}</div>
			</div>`;
		const printers = (rep.printers || []).map((p, i) => row(p,
			`<a class="axf-btn axf-btn-new" data-use="${i}">${__("Use this printer")}</a>`)).join("")
			|| `<div class="axf-row-detail">${__("No printers answered on 9100, 515 or 631.")}</div>`;
		const bridges = (rep.bridges || []).map((b) => row(b)).join("")
			|| `<div class="axf-row-detail">${__("No other bridges with LAN access on this network.")}</div>`;
		const others = (rep.others || []).map((o) => row(o)).join("");

		const d = new frappe.ui.Dialog({
			title: __("Network scan — {0}", [esc(raw.scope || "")]),
			size: "large",
			fields: [{ fieldtype: "HTML", fieldname: "b" }],
			primary_action_label: __("Close"),
			primary_action: () => { d.hide(); openSetup(); },
		});
		d.fields_dict.b.$wrapper.html(`
			<div class="axf-setup">
				<p class="axf-note">${__("Scanned {0} address(es) from {1} in {2}s. Recorded on terminal {3}.",
					[raw.scanned, esc(raw.from_ip || ""), raw.seconds, esc(terminal)])}</p>
				<h5>${__("Printers")}</h5>${printers}
				<h5>${__("AlphaX bridges")}</h5>${bridges}
				${others ? `<h5>${__("Other devices")}</h5>${others}` : ""}
				${(rep.unmatched_bridges || []).length ? `<p class="axf-note">${__("Bridges not matched to a terminal report their full hardware through their own heartbeat once that PC opens the cashier screen.")}</p>` : ""}
			</div>`);
		d.fields_dict.b.$wrapper.find("[data-use]").on("click", function () {
			const p = rep.printers[+$(this).data("use")];
			usePrinter(terminal, p);
		});
		d.show();
	}

	function usePrinter(terminal, p) {
		const base = (p.vendor ? p.vendor + " " : "") + (p.kind === "printer" ? "Printer" : "Device");
		frappe.prompt([
			{ fieldtype: "Data", fieldname: "name", label: __("Device name on the bridge"), reqd: 1,
			  default: `${base} ${p.ip.split(".").pop()}`.trim() },
			{ fieldtype: "Select", fieldname: "use", label: __("Use for"), reqd: 1,
			  options: "Receipt printer\nKitchen printer", default: "Receipt printer" },
		], async (v) => {
			const reg = await bridgeFetch("/devices/network", { method: "POST", timeout: 15000, body: {
				name: v.name, host: p.ip, port: p.print_port || 9100,
				profile: p.suggested_profile || "generic-network-escpos" } });
			if (reg.status !== 200) {
				return frappe.msgprint({ title: __("Bridge could not add the printer"), indicator: "red",
					message: frappe.utils.escape_html(reg.data.error || ("HTTP " + reg.status)) });
			}
			frappe.call({
				method: `${SETUP}.create_print_station_from_scan`,
				args: { terminal, device_name: v.name, ip: p.ip, port: p.print_port || 9100, station_type: "Printer" },
				callback: (r) => {
					const m = r.message || {};
					frappe.show_alert({ indicator: "green", message:
						__("{0} on the bridge; print station {1} {2}.", [esc2(v.name), esc2(m.station || ""),
							m.created ? __("created") : __("updated")]) }, 8);
					load();
				},
			});
		}, __("Use {0}", [p.ip]), __("Add"));
	}
	const esc2 = (x) => frappe.utils.escape_html(String(x || ""));
