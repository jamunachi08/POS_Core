...
@frappe.whitelist(allow_guest=True)
def get_qr_menu(token: str):
    """Public menu for the QR table-ordering / kiosk web page.

    Validates the token, resolves the outlet, and returns sellable items
    (scoped to the outlet domain's default item group, like submit_qr_order)
    priced from Item Price (selling, outlet price list). The client never sees
    or dictates a rate other than the server-resolved one.
    """
    try:
        if not token:
            return {"ok": False, "message": "Missing token"}
        ...
        token_doc = tok[0]
        table_name = token_doc.get("table")
        outlet_name = token_doc.get("outlet") or (
            frappe.db.get_value("AlphaX POS Table", table_name, "outlet")
        ...
        if outlet_name:
            outlet = frappe.db.get_value(
                "AlphaX POS Outlet",
                outlet_name,
                ["outlet_name", "company", "default_price_list", "primary_domain"],
                as_dict=True,
            ) or {}

        price_list = outlet.get("default_price_list")
        default_item_group = None
        if outlet.get("primary_domain"):
            default_item_group = frappe.db.get_value(
                "AlphaX POS Domain Pack", outlet["primary_domain"], "default_item_group"
            )
        ...
        currency = "SAR"
        if outlet.get("company"):
            currency = frappe.db.get_value(
                "Company", outlet["company"], "default_currency"
            ) or "SAR"
        ...
        for it in rows:
            ...
            if price_list:
                rate = frappe.db.get_value(
                    "Item Price",
                    {"item_code": it["item_code"], "price_list": price_list, "selling": 1},
                    "price_list_rate",
                ) or 0.0
            if not rate:
                rate = it.get("standard_rate") or 0.0
            rate = float(rate)
            if rate <= 0:
                continue
            items.append({
        ...
        groups, seen = [], set()
        for it in items:
            if it["item_group"] not in seen:
                seen.add(it["item_group"])
                groups.append(it["item_group"])

        return {
            "ok": True,
            "outlet": outlet.get("outlet_name") or outlet_name or "",
            "table": table_name or "",
            "currency": currency,
            "groups": groups,
            "items": items,
        ...
        frappe.log_error(frappe.get_traceback(), "AlphaXPOS QR Menu Failed")
        return {"ok": False, "message": "Menu unavailable"}


@frappe.whitelist()
def ensure_table_token(table: str, rotate: int = 0):
    """Create or rotate an active QR ordering token for a table and return the
    public ordering URL + printable QR URL. Staff-only (login required)."""
    if not table or not frappe.db.exists("AlphaX POS Table", table):
    ...
    active = frappe.get_all(
        "AlphaX POS QR Table Token",
        filters={"table": table, "is_active": 1},
        fields=["name", "token"],
    ...
    if active and not rotate:
        ...
        if rotate:
            for nm in frappe.get_all(
                ...
                frappe.db.set_value("AlphaX POS QR Table Token", nm, "is_active", 0)
        token = frappe.generate_hash(length=22)
        doc = frappe.get_doc({
            "doctype": "AlphaX POS QR Table Token",
            "table": table,
            "outlet": outlet,
            "token": token,
            "is_active": 1,
    ...
    return {
        ...
        "table": table,
        "outlet": outlet,
        "order_url": base + "/bonanza_order?token=" + token,
        "print_url": base + "/table_qr?token=" + token,
    }


@frappe.whitelist()
def generate_outlet_table_tokens(outlet: str):
    """Ensure every table in an outlet has an active token; return a print-all URL."""
    if not outlet:
        return {"ok": False, "message": "Missing outlet"}
    tables = frappe.get_all("AlphaX POS Table", filters={"outlet": outlet}, pluck="name")
    done = 0
    for t in tables:
        if ensure_table_token(t, rotate=0).get("ok"):
            done += 1
    base = frappe.utils.get_url()
    return {"ok": True, "tables": done, "print_all_url": base + "/table_qr?outlet=" + outlet}
...
@frappe.whitelist()
def qr_diagnose(token: str = "", table: str = ""):
    """Why is the QR menu empty?

    Every filter get_qr_menu applies, reported one at a time with the count
    that survives it. Manager-only: it exposes outlet configuration, and a
    guest has no business reading it.

    Run from the desk console or the browser:
        /api/method/alphax_pos_suite.alphax_pos_suite.api.qr_diagnose?token=...
    """
    if not any(r in frappe.get_roles() for r in
               ("System Manager", "AlphaX POS Manager")):
        frappe.throw(frappe._("Manager role required."), frappe.PermissionError)

    out = {"token": token or None, "table": table or None, "checks": []}
    ...
    if not token and table:
        token = frappe.db.get_value(
            "AlphaX POS QR Table Token", {"table": table, "is_active": 1}, "token") or ""
        out["token"] = token or None
        note("token found for table", bool(token),
             "No active token — open the table and print the order QR first.")

    if not token:
        note("token supplied", False, "Pass ?token=... or ?table=...")
        return out

    tok = frappe.db.get_value(
        "AlphaX POS QR Table Token", {"token": token},
        ["name", "table", "outlet", "is_active", "last_used_on"], as_dict=True)
    if not note("token exists", bool(tok)):
        return out
    if not note("token active", tok.is_active, "Tick Is Active on the token."):
        return out

    out["table"] = tok.table
    outlet_name = tok.outlet or (
        frappe.db.get_value("AlphaX POS Table", tok.table, "outlet") if tok.table else None)
    out["outlet"] = outlet_name
    if not note("outlet resolved", bool(outlet_name),
                "Set Outlet on the table or on the token."):
        return out

    outlet = frappe.db.get_value(
        "AlphaX POS Outlet", outlet_name,
        ["default_price_list", "primary_domain", "company"], as_dict=True) or {}
    out["price_list"] = outlet.get("default_price_list")
    note("outlet has a price list", bool(outlet.get("default_price_list")),
         "Without one the menu falls back to Item.standard_rate; items priced "
         "only in a price list will show as unavailable.")

    default_group = None
    if outlet.get("primary_domain"):
        default_group = frappe.db.get_value(
            "AlphaX POS Domain Pack", outlet["primary_domain"], "default_item_group")
    out["default_item_group"] = default_group
    note("domain default item group", bool(default_group),
         "Blank means the whole item tree is offered — usually fine, but it is "
         "why an unexpected item can appear.")

    filters = {"disabled": 0, "has_variants": 0, "is_sales_item": 1}
    sellable = frappe.get_all("Item", filters=filters, pluck="name", limit=2000)
    out["sellable_items"] = len(sellable)
    note("sellable items exist", bool(sellable))

    in_group = sellable
    if default_group:
        bounds = frappe.db.get_value("Item Group", default_group, ["lft", "rgt"], as_dict=True)
        if bounds:
            in_group = frappe.get_all(
                "Item", filters=dict(filters, item_group=["in", frappe.get_all(
                    "Item Group",
                    filters={"lft": [">=", bounds.lft], "rgt": ["<=", bounds.rgt]},
                    pluck="name")]), pluck="name", limit=2000)
    out["items_in_group"] = len(in_group)
    note("items inside the domain group", bool(in_group),
         f"{len(sellable)} sellable items, {len(in_group)} under {default_group}.")

    priced = 0
    unpriced = []
    for code in in_group[:500]:
        rate = 0
        if outlet.get("default_price_list"):
            rate = frappe.db.get_value(
                "Item Price",
                {"item_code": code, "price_list": outlet["default_price_list"], "selling": 1},
                "price_list_rate") or 0
        if not rate:
            rate = frappe.db.get_value("Item", code, "standard_rate") or 0
        if rate:
            priced += 1
        elif len(unpriced) < 10:
            unpriced.append(code)
    out["priced_items"] = priced
    out["unpriced_sample"] = unpriced
    note("items carry a price", priced > 0,
         "Items with no Item Price and no standard rate are dropped from the "
         "menu — this is the usual reason a scanned QR shows nothing.")
