...
# Pre-v15.10.7 hard-coded modes. Kept as the fallback so a site that has
# not migrated yet still boots a working register.
_LEGACY_ORDER_TYPES = [
        {"order_type_name": "Dine In", "icon": "\U0001f37d", "table_policy": "Optional",
         "opens_tab": 1, "prints_kot": 1, "is_default": 1, "sort_order": 10},
        {"order_type_name": "Takeaway", "icon": "\U0001f961",
         "table_policy": "Not Applicable", "prints_kot": 1, "sort_order": 20},
        {"order_type_name": "Delivery", "icon": "\U0001f6f5",
         "table_policy": "Not Applicable", "prints_kot": 1,
         "requires_delivery_platform": 1, "sort_order": 30},
        {"order_type_name": "Staff", "icon": "\U0001f9d1",
         "table_policy": "Not Applicable", "prints_kot": 1, "sort_order": 40},
        {"order_type_name": "Credit", "icon": "\U0001f4d2",
         "table_policy": "Not Applicable", "requires_customer": 1,
         "posts_credit": 1, "sort_order": 50},
    ]


def _order_types(outlet_name=None):
    """Service modes the register may offer, in display order.

    Outlet-scoped rows win over global rows of the same name, so one site
    can make Dine In mandatory-table while another leaves it optional.
    Falls back to the pre-v15.10.7 hard-coded five if the doctype has not
    been created yet — an unmigrated site must still be able to sell.
    """
    LEGACY = _LEGACY_ORDER_TYPES
    ...
    rows = frappe.get_all(
        ...
        filters={"enabled": 1},
        fields=[
            "order_type_name", "icon", "sort_order", "is_default", "outlet",
            "table_policy", "room_policy", "requires_customer",
            "requires_delivery_platform", "requires_cover_count",
            "requires_manager_pin", "prints_kot", "opens_tab", "posts_credit",
            "price_list", "service_charge_percent", "target_prep_minutes",
        ],
    ...
    # Outlet-specific row shadows the global row of the same name.
    by_name = {}
    for r in rows:
        if r.get("outlet") and r["outlet"] != outlet_name:
            continue
        held = by_name.get(r["order_type_name"])
        if held and held.get("outlet") and not r.get("outlet"):
            continue
        by_name[r["order_type_name"]] = r

    out = sorted(by_name.values(), key=lambda r: (r.get("sort_order") or 0,
                                                  r["order_type_name"]))
...
def pos_boot(terminal):
    ...
    # v15.10.7 — service modes. The register decides whether to ask for a
    # table, a customer, a platform or a room entirely from this.
    payload["order_types"] = _section(
        "order_types", lambda: _order_types(outlet_name), _LEGACY_ORDER_TYPES)
