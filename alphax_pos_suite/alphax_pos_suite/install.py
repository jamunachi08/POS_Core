...
def seed_order_types():
    """Seed the service modes.

    The five names match what the register hard-coded before v15.10.7, so
    existing invoices keep reporting continuity. Flags reproduce the old
    behaviour exactly, with one correction the sites kept asking for:
    Takeaway and Delivery no longer ask for a table.

    Room Service ships disabled — it is the hospitality mode, and a
    restaurant that never uses it should not see a button for it.
    Idempotent: an operator who has tuned a row keeps their tuning.
    """
    defaults = [
        # name, icon, sort, table_policy, flags
        ("Dine In", "\U0001f37d", 10, "Mandatory", {
            "opens_tab": 1, "prints_kot": 1, "requires_cover_count": 1,
            "target_prep_minutes": 15, "is_default": 1,
        }),
        ("Takeaway", "\U0001f961", 20, "Not Applicable", {
            "prints_kot": 1, "target_prep_minutes": 10,
        }),
        ("Delivery", "\U0001f6f5", 30, "Not Applicable", {
            "prints_kot": 1, "requires_delivery_platform": 1,
            "target_prep_minutes": 12,
        }),
        ("Staff", "\U0001f9d1", 40, "Not Applicable", {
            "prints_kot": 1, "requires_manager_pin": 1,
        }),
        ("Credit", "\U0001f4d2", 50, "Not Applicable", {
            "prints_kot": 1, "requires_customer": 1, "posts_credit": 1,
        }),
        ("Room Service", "\U0001f6cf", 60, "Not Applicable", {
            "prints_kot": 1, "opens_tab": 1, "room_policy": "Mandatory",
            "target_prep_minutes": 25, "enabled": 0,
        }),
    ]
    if not frappe.db.table_exists("AlphaX POS Order Type"):
        return
    for name, icon, sort_order, table_policy, flags in defaults:
        try:
            if frappe.db.exists("AlphaX POS Order Type", name):
                continue
            doc = {
                "doctype": "AlphaX POS Order Type",
            ...
            frappe.log_error(
                title=f"AlphaX POS: could not seed order type {name}",
                message=frappe.get_traceback(),
            )


def sync_order_type_field_options():
    """Keep the Sales Invoice order-type Select in step with the modes.

    The field is a Select, not a Link, so an operator adding "Room Service"
    or "Drive Thru" would otherwise post invoices that fail validation. Run
    after seeding and on every migrate; cheap and idempotent.
    """
    if not frappe.db.table_exists("AlphaX POS Order Type"):
        return
    try:
        names = frappe.get_all(
            ...
            filters={"enabled": 1},
            pluck="order_type_name",
            order_by="sort_order asc, order_type_name asc",
        )
        if not names:
            return
        # Disabled modes stay in the options list: historical invoices
        # carry those values and must remain readable.
        existing = frappe.db.get_value(
            "Custom Field", {"dt": "Sales Invoice", "fieldname": "alphax_order_type"},
            ["name", "options"], as_dict=True,
        )
        if not existing:
            return
        held = [o for o in (existing.options or "").split("\n") if o.strip()]
        merged = list(dict.fromkeys(held + names))
        options = "\n" + "\n".join(merged)
        if options != (existing.options or ""):
            frappe.db.set_value("Custom Field", existing.name, "options", options)
            frappe.clear_cache(doctype="Sales Invoice")
    except Exception:
        frappe.log_error(
            title="AlphaX POS: could not sync order type field options",
            message=frappe.get_traceback(),
        )


def seed_outlet_menus():
    """Give every outlet a menu that reproduces what it already sold.

    Migration principle: an upgrade must change behaviour only where the
    operator asks it to. Each outlet with no menu gets one built from its
    domain's default item group — the exact set the old code resolved —
    so day one after this release looks identical, and the operator can
    then narrow or split it per branch at their own pace.

    Idempotent, and never touches an outlet that already has menus.
    """
    if not frappe.db.table_exists("AlphaX POS Menu"):
        return
    try:
        outlets = frappe.get_all("AlphaX POS Outlet",
                                 fields=["name", "outlet_name", "primary_domain", "company"])
    except Exception:
        return
    ...
    for o in outlets:
        try:
            existing = frappe.get_all(
                ...
                filters={"parenttype": "AlphaX POS Outlet", "parent": o.name},
                limit=1)
            if existing:
                continue
            ...
            if o.get("primary_domain"):
                group = frappe.db.get_value(
                    "AlphaX POS Domain Pack", o["primary_domain"], "default_item_group")
            if not group:
                group = frappe.db.get_value("Item Group", {"is_group": 1, "parent_item_group": ""},
                                            "name") or "All Item Groups"
            if not frappe.db.exists("Item Group", group):
                continue

            menu_name = f"{o.outlet_name or o.name} Menu"
            if not frappe.db.exists("AlphaX POS Menu", menu_name):
                frappe.get_doc({
                    "doctype": "AlphaX POS Menu",
                    "menu_name": menu_name,
            ...
            outlet = frappe.get_doc("AlphaX POS Outlet", o.name)
            outlet.append("menus", {"menu": menu_name})
            outlet.save(ignore_permissions=True)
        except Exception:
            frappe.log_error(
                title=f"AlphaX POS: could not seed menu for outlet {o.name}",
                message=frappe.get_traceback())
...
def after_install():
    ...
    heal_lost_manager_access()
    seed_delivery_platforms()
    seed_order_types()
    seed_outlet_menus()
    sync_order_type_field_options()
    create_roles()
    ...
    # v15.15.0 — the process flow board. Runs last so every doctype it
    # points at already exists; a node whose target is missing is dropped
    # at read time rather than here, so a partial install still renders.
    ...
    try:
        ...
        frappe.log_error(
            title="AlphaX POS: process flow seeding failed",
            message=frappe.get_traceback(),
        )
...
def create_roles():
    ...
    roles = [
        ...
        "AlphaX POS Manager",
        # v15.14.0 — AlphaX POS KDS Ticket grants read/write/create to this
        # role in its doctype JSON, but nothing ever created it. On a fresh
        # site the permission row pointed at a non-existent Role, so no
        # kitchen user could be granted KDS access without hand-creating it.
        "AlphaX POS Kitchen",
...
def apply_permissions():
    ...
    CAREFUL — the trap this walked into once already: the moment a single
    Custom DocPerm row exists for a doctype, Frappe *replaces* the whole
    standard permission set from the DocType JSON with the custom rows.
    Any role not listed here loses access silently, and the symptom is not
    "not permitted" but a desk route that no longer resolves:

        Page alphax-pos-settings not found

    System Manager must therefore appear in every entry below. Administrator
    is unaffected (it bypasses permissions), which is exactly why this hides
    from whoever set the site up and only bites a real user.
    """
    ...
    perm_map = {
        "AlphaX POS Settings": {
            "System Manager": {"read": 1, "write": 1, "create": 1, "delete": 1, "submit": 0, "cancel": 0},
...
def heal_lost_manager_access():
    """Repair sites already locked out by the missing System Manager row.

    Adding the role to perm_map fixes new installs; it does nothing for a
    site whose Custom DocPerm rows were written before this release. This
    runs on every migrate: wherever custom perms exist for one of our
    doctypes and System Manager is absent, the row is restored and the
    permission cache cleared.

    Also covers the general case — any AlphaX doctype that somehow ended up
    with custom perms and no System Manager — because a desk route that
    silently stops resolving is close to impossible to diagnose from the
    error message Frappe shows.
    """
    if not frappe.db.exists("DocType", "Custom DocPerm"):
        return

    try:
        governed = frappe.get_all(
            "Custom DocPerm", filters={"permlevel": 0},
    ...
    healed = []
    for doctype in governed:
        if not str(doctype).startswith("AlphaX"):
            continue
        ...
        try:
            ...
            healed.append(doctype)
        except Exception:
            frappe.log_error(
                title=f"AlphaX POS: could not restore System Manager perm on {doctype}",
                message=frappe.get_traceback(),
            )
