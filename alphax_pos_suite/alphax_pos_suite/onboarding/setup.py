...
TERMINAL_FIELDS = [
    ...
    # v15.17.0 — LAN discovery. Written by report_network_scan after the
    # bridge on this PC scans the shop network. The server itself can never
    # reach a private address; only a bridge inside the network can.
    dict(fieldname="section_network_scan", label="Network Discovery",
         fieldtype="Section Break", insert_after="hardware_configured_on",
         collapsible=1,
         description="Printers and other AlphaX bridges found on this PC's "
                     "network. Run from the process flow board: Setup & Install "
                     "> Scan network. Nothing here is fetched by the server."),
    dict(fieldname="bridge_network_scanned_on", label="Last Scanned",
         fieldtype="Datetime", insert_after="section_network_scan", read_only=1),
    dict(fieldname="bridge_network_scope", label="Scanned Range",
         fieldtype="Data", insert_after="bridge_network_scanned_on", read_only=1),
    dict(fieldname="bridge_network_found", label="Devices Found",
         fieldtype="Int", insert_after="bridge_network_scope", read_only=1),
    dict(fieldname="bridge_network_json", label="Scan Result",
         fieldtype="Code", options="JSON", insert_after="bridge_network_found",
         read_only=1),
]
