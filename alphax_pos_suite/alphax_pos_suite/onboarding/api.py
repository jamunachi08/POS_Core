...
def _terminal_identity(doc) -> dict:
    return {
        ...
        "hardware_profile": doc.get("hardware_profile"),
        "hardware_configured": bool(doc.get("hardware_configured_on")),
    }
...
#
# A station is not "missing hardware" — it has exactly the hardware the
# operator decided it has. A tablet taking orders on a terrace needs no
# bridge at all; a full till needs five devices. The wizard asks once, and
# from then on only the ticked roles are set up, monitored, or complained
# about.
# ---------------------------------------------------------------------------
...
HARDWARE_ROLES = [
    {
        ...
        "sublabel": "Thermal printer for the customer's bill",
        "needs_bridge": 1,
        "default_on": 1,
        "group": "print",
    },
    {
        ...
        "sublabel": "Prints kitchen tickets. One printer can serve every section.",
        "needs_bridge": 1,
        "default_on": 0,
        "group": "print",
    },
    {
        "id": "cash_drawer",
        "label": "Cash drawer",
        "sublabel": "Opens on cash tender. Card-only tills do not need one.",
        "needs_bridge": 1,
        "default_on": 1,
        "group": "cash",
    },
    {
        "id": "card_terminal",
        "label": "Card terminal",
        "sublabel": "Integrated payment device (Geidea, Neoleap, PayTabs)",
        "needs_bridge": 1,
        "default_on": 0,
        "group": "cash",
    },
    {
        ...
        "sublabel": "Keyboard-wedge scanners work with no bridge at all",
        "needs_bridge": 0,
        "default_on": 0,
        "group": "input",
    },
    {
        ...
        "sublabel": "Serial or USB scale for loose items",
        "needs_bridge": 1,
        "default_on": 0,
        "group": "input",
    },
    {
        ...
        "sublabel": "Second screen or pole display facing the guest",
        "needs_bridge": 1,
        "default_on": 0,
        "group": "display",
    },
    {
        ...
        "sublabel": "Screen in the kitchen instead of (or beside) paper",
        "needs_bridge": 0,
        "default_on": 0,
        "group": "display",
    },
...
HARDWARE_PROFILES = [
    {
        "id": "Full Station",
        "label": "Full till",
        "sublabel": "Receipt printer, cash drawer, kitchen printer",
        "roles": ["receipt_printer", "cash_drawer", "kot_printer"],
    },
    {
        "id": "Receipt Only",
        "label": "Receipt only",
        "sublabel": "Prints the bill, nothing else attached",
        "roles": ["receipt_printer"],
    },
    {
        "id": "Tablet Only",
        "label": "Tablet / no hardware",
        "sublabel": "Order taking only — no bridge needed",
        "roles": [],
    },
    {
        "id": "Kitchen Only",
        "label": "Kitchen station",
        "sublabel": "Kitchen printer and display, no cash handling",
        "roles": ["kot_printer", "kds_screen"],
    },
]


def _default_plan() -> dict:
    return {r["id"]: bool(r["default_on"]) for r in HARDWARE_ROLES}


def _read_plan(doc) -> dict:
    raw = doc.get("hardware_plan_json")
    plan = _default_plan()
    if not raw:
        return plan
    try:
        stored = frappe.parse_json(raw) if isinstance(raw, str) else raw
        if isinstance(stored, dict):
            for r in HARDWARE_ROLES:
                if r["id"] in stored:
                    plan[r["id"]] = bool(stored[r["id"]])
    except Exception:
        pass
    return plan
...
@frappe.whitelist()
def get_hardware_catalog(terminal: str | None = None):
    """Roles the operator can tick, plus this terminal's current choice.

    `needs_bridge` is the field the wizard actually acts on: if nothing
    ticked needs the bridge, the whole install step is skipped rather than
    nagging a station that has no peripherals to reach.
    """
    plan = _default_plan()
    profile = None
    configured_on = None
    ...
    if terminal and frappe.db.exists("AlphaX POS Terminal", terminal):
        doc = frappe.get_doc("AlphaX POS Terminal", terminal)
        plan = _read_plan(doc)
        profile = doc.get("hardware_profile")
        configured_on = doc.get("hardware_configured_on")

    return {
        "roles": HARDWARE_ROLES,
        "profiles": HARDWARE_PROFILES,
        "plan": plan,
        "profile": profile,
        "configured_on": configured_on,
        "configured": bool(configured_on),
        "needs_bridge": any(
            plan.get(r["id"]) and r["needs_bridge"] for r in HARDWARE_ROLES
        ),
    }


@frappe.whitelist()
def save_hardware_plan(terminal: str, plan=None, profile: str | None = None):
    """Record what this station has. Re-runnable: the cashier can reopen
    the picker and add a cash drawer six months later."""
    if not frappe.db.exists("AlphaX POS Terminal", terminal):
        frappe.throw(_("Unknown terminal {0}").format(terminal))

    if terminal not in _allowed_terminals_for_user():
        frappe.throw(_("You are not allowed to configure {0}").format(terminal),
                     frappe.PermissionError)

    incoming = frappe.parse_json(plan) if isinstance(plan, str) else (plan or {})
    if not isinstance(incoming, dict):
        frappe.throw(_("Invalid hardware plan"))

    known = {r["id"] for r in HARDWARE_ROLES}
    clean = {k: bool(v) for k, v in incoming.items() if k in known}

    valid_profiles = {p["id"] for p in HARDWARE_PROFILES} | {"Custom"}
    if profile and profile not in valid_profiles:
        profile = "Custom"

    doc = frappe.get_doc("AlphaX POS Terminal", terminal)
    doc.set("hardware_plan_json", frappe.as_json(clean))
    doc.set("hardware_profile", profile or "Custom")
    doc.set("hardware_configured_on", now_datetime())
    doc.save(ignore_permissions=True)

    _log_event(terminal, "hardware_plan", {"plan": clean, "profile": profile})

    needs_bridge = any(
        clean.get(r["id"]) and r["needs_bridge"] for r in HARDWARE_ROLES
    )
    return {"ok": True, "plan": clean, "profile": doc.get("hardware_profile"),
