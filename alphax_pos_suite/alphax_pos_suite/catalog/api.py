...
def list_menu_items(terminal: str | None = None, outlet: str | None = None,
    ...
    chain = price_list_chain(outlet=outlet, terminal=terminal, menus=menus)
    _apply_prices(rows, chain)
    ...
    return {
        ...
        "price_list": chain[0][0] if chain else None,
        "price_chain": [{"price_list": pl, "source": src} for pl, src in chain],
        "unpriced": sum(1 for r in rows if r.get("price_source") == "unpriced"),
    }
...
def price_list_chain(outlet: str | None = None, terminal: str | None = None,
                     pos_profile: str | None = None, menus: list | None = None) -> list:
    """Ordered (price_list, source) candidates, most authoritative first.

    One chain, used by the tapped grid AND the barcode scanner, so an item
    can never cost one thing when tapped and another when scanned. Before
    v15.17.0 the grid read menu -> outlet -> standard rate while the scanner
    read ERPNext POS Profile -> outlet -> Selling Settings; on a site whose
    POS Profile and Outlet named different price lists, the same item
    carried two prices depending on how the cashier added it.

      1. menu price lists, highest menu priority first
      2. the outlet's default price list
      3. the ERPNext POS Profile's selling price list (terminal's profile)
      4. Selling Settings default selling price list
      then the Item's standard rate, then 0 and flagged "unpriced".
    """
    chain = []
    for m in sorted(menus or [], key=lambda x: -cint(x.get("priority"))):
        if m.get("price_list"):
            chain.append((m["price_list"], f"menu:{m.get('menu_name') or m.get('name')}"))

    if outlet:
        pl = frappe.db.get_value("AlphaX POS Outlet", outlet, "default_price_list")
        if pl:
            chain.append((pl, "outlet"))

    if not pos_profile and terminal:
        pos_profile = frappe.db.get_value("AlphaX POS Terminal", terminal, "pos_profile")
    ...
    try:
        ...
        pl = None
    if pl:
        chain.append((pl, "selling_settings"))
...
def _apply_prices(rows: list, chain: list):
    """Resolve each row's rate by walking ``chain``; first list with a price wins."""
    if not rows:
        return
    priced = {}
    pending = {r["item_code"] for r in rows}
    for pl, src in chain:
        if not pending:
            break
        for p in frappe.get_all(
            "Item Price",
            filters={"price_list": pl, "selling": 1, "item_code": ["in", list(pending)]},
            fields=["item_code", "price_list_rate"],
        ):
            if p.item_code in pending:
                priced[p.item_code] = (p.price_list_rate, pl, src)
                pending.discard(p.item_code)

    for r in rows:
        rate, pl, src = priced.get(r["item_code"], (None, None, None))
        if rate is not None:
            r["rate"], r["price_list"], r["price_source"] = rate, pl, src
        elif r.get("standard_rate"):
            r["rate"], r["price_list"], r["price_source"] = r["standard_rate"], None, "standard_rate"
        else:
            r["rate"], r["price_list"], r["price_source"] = 0, None, "unpriced"


def rate_for(item_code: str, outlet: str | None = None, terminal: str | None = None,
             pos_profile: str | None = None, standard_rate: float = 0) -> tuple:
    """Single-item rate on the same chain as the grid. Used by the scanner."""
    menus = menus_for(terminal=terminal, outlet=outlet) if outlet else []
    row = {"item_code": item_code, "standard_rate": standard_rate}
    _apply_prices([row], price_list_chain(outlet, terminal, pos_profile, menus))
    return row["rate"], row["price_list"], row["price_source"]
...
def catalog_diagnose(terminal: str):
    ...
    chain = price_list_chain(outlet=t["pos_outlet"], terminal=terminal, menus=live)
    out["price_chain"] = [{"price_list": pl, "source": src} for pl, src in chain]
    note("a price list is in force", bool(chain),
         " -> ".join(f"{pl} ({src})" for pl, src in chain)
         or "None anywhere: every item falls back to its standard rate, usually 0.")

    scope = list(codes if codes is not None else legacy)[:2000]
    if scope:
        rows = frappe.get_all("Item", filters={"name": ["in", scope]},
                              fields=["item_code", "standard_rate"])
        _apply_prices(rows, chain)
        unpriced = [r["item_code"] for r in rows if r["price_source"] == "unpriced"]
        out["unpriced"] = len(unpriced)
        note("every item has a price", not unpriced,
             f"{len(unpriced)} of {len(rows)} would sell at 0.00, e.g. {unpriced[:5]}"
             if unpriced else f"all {len(rows)} priced")

    if out.get("source") == "domain_group":
        grp = None
        dom = frappe.db.get_value("AlphaX POS Outlet", t["pos_outlet"], "primary_domain")
        if dom:
            grp = frappe.db.get_value("AlphaX POS Domain Pack", dom, "default_item_group")
        note("catalogue is scoped", bool(grp),
             f"domain group '{grp}'" if grp else
             "No menu and no domain item group: the till shows EVERY sellable item on "
             "the site, including items created by other apps. Assign a menu to the outlet.")
