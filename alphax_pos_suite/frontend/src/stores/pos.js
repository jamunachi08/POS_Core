...
export const usePOSStore = defineStore('pos', () => {
  ...
  async function loadBoot() {
    ...
    try {
      ...
      // The mode the register opens in is configuration, not a constant.
      // Also guards against a mode that was disabled since the last sale.
      if (!ORDER_TYPES.value.includes(orderType.value)) {
        orderType.value = defaultOrderType.value
      }
  ...
  // ---- menu loading -----------------------------------------------------
  const menuItems = ref([])
  // Where the range came from, so the till can say so rather than leaving
  // an empty grid to be interpreted.
  const menuSource = ref(null)     // 'menu' | 'domain_group' | 'legacy_client'
  const menuWarning = ref(null)
  ...
  async function loadMenu() {
    ...
    try {
      ...
      // v15.12 — ask the server what this till may sell. The menu is a
      // property of the outlet, resolved server-side; the browser no
      // longer derives it from the domain's item group, which could only
      // ever give every Restaurant outlet the same range.
      let rows = null
      try {
        const res = await api.menuItems({
          terminal: terminal.value,
          outlet: boot.value?.outlet?.name,
        })
        if (res && Array.isArray(res.items)) {
          rows = res.items
          menuSource.value = res.source || null
          menuWarning.value = res.warning || null
          menuTotal.value = res.total || rows.length
          menuNames.value = res.menus || []
        }
      } catch (e) {
        rows = null   // older server, or no permission — fall through
      }

      if (rows === null) {
        // Legacy path, kept so a till never loses its menu mid-upgrade.
        ...
        rows = await api.listItems({ item_groups: groupFilter })
        menuSource.value = 'legacy_client'
      }
  ...
  /** Start-of-day gate: this till may not be used until a shift is open.
   *  Lives in the store rather than the view so the policy is one
   *  expression, testable, and identical wherever it is consulted. */
  const shiftGateActive = computed(() =>
    !!settings.value.require_shift_open
    && (settings.value.shift_gate || 'On Entry') !== 'On First Sale'
    && !shiftLoading.value
    && !shift.value)
  ...
  // Service modes come from the server (AlphaX POS Order Type) so an
  // operator can decide per outlet whether a table is required, which
  // modes exist at all, and what each one demands before it can be paid.
  // The hard-coded five remain as the fallback for an unmigrated site.
  const LEGACY_ORDER_TYPES = [
    { order_type_name: 'Dine In',  icon: '\u{1F37D}', table_policy: 'Optional',
      opens_tab: 1, prints_kot: 1, is_default: 1, sort_order: 10 },
    { order_type_name: 'Takeaway', icon: '\u{1F961}', table_policy: 'Not Applicable',
      prints_kot: 1, sort_order: 20 },
    { order_type_name: 'Delivery', icon: '\u{1F6F5}', table_policy: 'Not Applicable',
      prints_kot: 1, requires_delivery_platform: 1, sort_order: 30 },
    { order_type_name: 'Staff',    icon: '\u{1F9D1}', table_policy: 'Not Applicable',
      prints_kot: 1, sort_order: 40 },
    { order_type_name: 'Credit',   icon: '\u{1F4D2}', table_policy: 'Not Applicable',
      requires_customer: 1, posts_credit: 1, sort_order: 50 },
  ]

  const orderTypeDefs = computed(() => {
    const rows = boot.value?.order_types
    return Array.isArray(rows) && rows.length ? rows : LEGACY_ORDER_TYPES
  })
  const ORDER_TYPES = computed(() => orderTypeDefs.value.map(r => r.order_type_name))

  const defaultOrderType = computed(() => {
    const flagged = orderTypeDefs.value.find(r => r.is_default)
    return (flagged || orderTypeDefs.value[0] || {}).order_type_name || 'Dine In'
  })

  const orderType = ref('Dine In')

  /** Flags for a mode; an unknown name yields an all-off definition
   *  rather than undefined, so callers never need a null guard. */
  function orderTypeDef(name) {
    return orderTypeDefs.value.find(r => r.order_type_name === name) || {
      order_type_name: name, table_policy: 'Not Applicable',
    }
  }
  const activeOrderType = computed(() => orderTypeDef(orderType.value))

  // 'Not Applicable' | 'Optional' | 'Mandatory'. A venue with no floor
  // plan never asks for a table regardless of what the mode says.
  const tablePolicy = computed(() => {
    if (!activeFeatures.value.uses_floor_plan) return 'Not Applicable'
    return activeOrderType.value.table_policy || 'Not Applicable'
  })
  const tableRequired = computed(() => tablePolicy.value === 'Mandatory')
  const tableApplies = computed(() => tablePolicy.value !== 'Not Applicable')
  const roomPolicy = computed(() => activeOrderType.value.room_policy || 'Not Applicable')
  const roomRequired = computed(() => roomPolicy.value === 'Mandatory')

  /** What still has to be supplied before this order can be paid.
   *  Empty array = clear to tender. The register shows the first one. */
  const blockingRequirements = computed(() => {
    const d = activeOrderType.value
    const out = []
    if (tableRequired.value && !activeTable.value) out.push('table')
    if (d.requires_customer && !customer.value) out.push('customer')
    if (d.requires_delivery_platform && !deliveryPlatform.value) out.push('platform')
    if (roomRequired.value && !context.value.room) out.push('room')
    if (d.requires_cover_count && !context.value.covers) out.push('covers')
    return out
  })
  ...
  function setOrderType(type, platform = null) {
    if (!ORDER_TYPES.value.includes(type)) return
    orderType.value = type
    deliveryPlatform.value = orderTypeDef(type).requires_delivery_platform
      ? platform
      : null
    // Takeaway and delivery have no table. Holding one from a previous
    // dine-in sale would silently seat a walk-out order.
    if (tablePolicy.value === 'Not Applicable') activeTable.value = null
  }
  ...
  function clearCart() {
    ...
    context.value = { rx_number: null, doctor: null, patient: null, batch: null,
                      appointment: null, room: null, covers: null }
  }
  ...
  function setTable(tableName) {
    // A mode that does not seat guests must never end up holding a table.
    if (tableName && tablePolicy.value === 'Not Applicable') return
    activeTable.value = tableName
  }
  function setCovers(n) {
    const v = Number(n)
    context.value.covers = Number.isFinite(v) && v > 0 ? Math.round(v) : null
  }
  function setRoom(room) { context.value.room = room || null }
  ...
  const context = ref({
    ...
    batch: null,
    appointment: null,
    // v15.10.7 — hospitality and dine-in slots. `room` carries the folio
    // a charge posts to; `covers` is the seated guest count.
    room: null,
    covers: null,
  })
  ...
  async function submitSale() {
    ...
    // Settlement route comes from the mode's flags, not its name, so a
    // site can add "House Account" or "Aggregator — Careem" and have it
    // behave correctly without a code change.
    const modeDef = orderTypeDef(orderType.value)
    const isCredit = !isReturn && !!modeDef.posts_credit
    const isDelivery = !isReturn && !!modeDef.requires_delivery_platform
    ...
    const invoice = {
      ...
      // Channel pricing: an aggregator menu usually carries a marked-up
      // list so commission does not come out of the margin.
      selling_price_list: (!isReturn && modeDef.price_list) || outlet.default_price_list,
      ...
      // The table belongs on the invoice, not only on the kitchen ticket:
      // covers-per-table, turn time and server performance all read it.
      alphax_table: isReturn ? null : (activeTable.value || null),
      alphax_covers: isReturn ? 0 : (Number(context.value.covers) || 0),
      alphax_room: isReturn ? null : (context.value.room || null),
  ...
  function routeCartToStations() {
    ...
    for (const line of routable) {
      ...
      if (!station) continue
      // Remember where the line WOULD have gone even when everything is
      // about to be merged onto one printer — it becomes the section
      // heading on the shared ticket.
      line.section = (stationByName[station] || {}).station_name || station
      ;(groups[station] = groups[station] || []).push(line)
    }

    // Many kitchens have exactly one printer. Routing them to four
    // stations that all resolve to the same device prints four tickets
    // and annoys the line. Merge into one, keeping the section labels so
    // the ticket can still be torn apart by hand.
    if (kot.mode === 'Single Printer') {
      const merged = Object.values(groups).flat()
      if (!merged.length) return []
      const target = kot.single_target
        || (fallback && (fallback.bridge_target || fallback.name))
      return [{
        station: {
          name: target || 'kitchen',
          station_name: (fallback && fallback.station_name) || 'KITCHEN',
          bridge_target: target || null,
        },
        lines: merged,
        merged: true,
        group_by_section: kot.group_by_section !== 0,
      }]
    }
  ...
  function buildKotTicket(stationGroup, invoiceName) {
    return {
      ...
      // On a shared printer the section a line belongs to is printed as a
      // heading above its block; the bridge renders `section` when the
      // value changes between consecutive lines.
      merged: !!stationGroup.merged,
      group_by_section: stationGroup.group_by_section !== false,
      items: stationGroup.lines.map(l => ({
        name: l.item_name,
        qty: l.qty,
        section: (stationGroup.merged && stationGroup.group_by_section !== false)
          ? (l.section || '') : '',
  ...
  return {
    ...
    menuItems, menuLoading, menuSource, menuWarning, menuTotal, menuNames,
    itemGroups, activeCategory,
    categories, filteredMenu, searchQuery,
    cart, cartUuid,
    settings, returnMode, returnReason, setReturnMode,
    ORDER_TYPES, orderTypeDefs, orderTypeDef, activeOrderType, defaultOrderType,
    tablePolicy, tableRequired, tableApplies, roomPolicy, roomRequired,
    blockingRequirements, setCovers, setRoom,
    orderType, deliveryPlatform, deliveryPlatforms, setOrderType,
    theme, setTheme,
    shift, shiftLoading, shiftGateActive, businessDate, combos, taxBreakdown, lineTax, loadShift, openShift, cashMovement, fetchXReport, closeShift, runDayClose,
