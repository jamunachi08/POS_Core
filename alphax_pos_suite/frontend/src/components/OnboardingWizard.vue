...
// Reaching 'ready' is what leaves the wizard, and it can be reached from
// six places: mount, claim, provision, hardware save, bridge recheck,
// token paste, skip. Every call site used to have to remember to call
// enter() itself, and saveHardware() (added with the hardware step)
// did not — the step became 'ready', no pane matched it, and the card
// sat on its header and step dots forever.
//
// A watcher is the fix rather than a seventh call: the component now
// reacts to the state, so no future transition can forget.
let entered = false
function enter() {
  if (entered) return                 // idempotent: watcher + call site
  if (!ob.terminal?.name) return      // nothing to hand the till
  entered = true
  ob.startHeartbeat()
  store.changeTerminal(ob.terminal.name)
}

watch(() => ob.step, (s) => { if (s === 'ready') enter() }, { immediate: true })

// A 'ready' state that somehow cannot enter (no bound terminal) would be
// the same dead card by another route. Say so rather than hang.
const stranded = computed(() => ob.step === 'ready' && !ob.terminal?.name)
...
<template>
  <div class="ob-screen">
    ...
    <div class="ob-card" :class="{ wide: ob.step === 'bridge' }">
      ...
      <!-- ===================== 4. READY ===================== -->
      <!-- Brief by design: enter() hands over to the till almost at once.
           It exists so the card always shows something. -->
      <section v-else-if="ob.step === 'ready'" class="pane center">
        <template v-if="stranded">
          <div class="err-title">{{ t('errors.boot_failed') }}</div>
          <p class="muted small">{{ t('onboarding.ready_no_terminal') }}</p>
          <button class="btn btn-primary lg full" @click="ob.detect()">
            {{ t('app.retry') }}
          </button>
        </template>
        <template v-else>
          <div class="spinner"></div>
          <div class="lead">{{ t('onboarding.opening_till') }}</div>
        </template>
      </section>
