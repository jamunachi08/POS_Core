...
import OrderTypeBar from './OrderTypeBar.vue'
...
const emit = defineEmits([
  ...
  'edit-modifiers', 'split-bill', 'line-actions', 'pick-delivery'
])
...
<template>
  <section class="cart-panel">
    <div v-if="store.returnMode" class="return-banner">
      <span class="return-icon">↩</span>
      <span class="return-text">
        {{ t('returns.banner') }}
        <b v-if="store.returnReason"> — {{ store.returnReason }}</b>
      </span>
      <button class="return-exit" @click="store.setReturnMode(false)">×</button>
    </div>
    <OrderTypeBar v-if="!store.returnMode"
      @pick-delivery="emit('pick-delivery')"
      @need-customer="emit('add-customer')" />
    ...
    <div class="cart-totals" v-if="store.cart.length > 0">
      ...
      <!-- Per-account tax rows: VAT and (for sheesha lines) the tobacco
           fee show separately, matching the GL account heads exactly. -->
      <div class="t-row" v-for="tr in store.taxBreakdown" :key="tr.account_head">
        <span>{{ tr.label }}</span>
        <span class="tnum">{{ fmt(tr.amount) }}</span>
      </div>
...
.action-row {
  ...
  gap: 8px;
  margin: 0 18px;
  /* Breathing room + tablet home-indicator safe area: the Pay button
     must never sit flush against (or under) the screen edge. */
  padding-block-end: max(18px, env(safe-area-inset-bottom, 0px));
}
.split-btn {
  padding: 16px;
  border-radius: var(--r-md);
...
.return-icon { font-size: 15px; }
.return-text { flex: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.return-exit {
  font-size: 16px; line-height: 1;
  color: var(--danger);
  padding: 2px 6px; border-radius: var(--r-sm, 6px);
}
.return-exit:hover { background: rgba(0,0,0,0.06); }
</style>
