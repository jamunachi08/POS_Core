...
const props = defineProps({
  initialMode: { type: String, default: 'auto' },
  // Start-of-day gate: the register is not usable until the shift opens.
...
// Counting the drawer at handover is a policy, not a universal truth.
// Where it is not policy, a blank float means zero rather than a
// disabled button nobody can explain.
const floatRequired = computed(() => !!store.settings?.shift_opening_float_required)
...
<template>
  <AppModal :title="t('shift.title')" size="lg" :blocking="blocking" @close="emit('close')">
    <div class="sh-body">
      ...
      <template v-if="mode === 'open'">
        ...
        <div v-if="blocking" class="sh-gate-note">{{ t('shift.gate_note') }}</div>
        <div class="sh-actions">
          <button v-if="!blocking" class="btn" @click="emit('close')">{{ t('app.cancel') }}</button>
          <button class="btn btn-primary"
            :disabled="busy || (floatRequired && openingCash === '')" @click="doOpen">
            {{ t('shift.open_btn') }}
          </button>
        </div>
      </template>
