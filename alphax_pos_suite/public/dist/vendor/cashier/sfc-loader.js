...
(function (global) {
  ...
  function transformScriptSetup(rewrittenScript, filename) {
    ...
    // The rewrite produces text STARTING with `(`. In statement
    // position (bare `defineEmits([...])` on its own line) ASI glues
    // that onto a previous unterminated expression, turning
    // `const x = computed(...)\n(__emits_decl=...)` into
    // `computed(...)(...)` → "computed(...) is not a function". This
    // project's house style omits semicolons, so the hazard is real.
    // In expression position (`const props = defineProps({...})`) a
    // semicolon would be a syntax error. So: two passes.
    // Pass 1 — statement position (line starts with the call): guard with `;`.
    body = body.replace(/(^|\n)([ \t]*)defineProps\s*\(/g, '$1$2;(__props_decl = ');
    body = body.replace(/(^|\n)([ \t]*)defineEmits\s*\(/g, '$1$2;(__emits_decl = ');
    // Pass 2 — remaining expression positions: plain parenthesized assignment.
    body = body.replace(/\bdefineProps\s*\(/g, '(__props_decl = ');
    body = body.replace(/\bdefineEmits\s*\(/g, '(__emits_decl = ');
    // Pass 3 — make the whole expression EVALUATE to the live binding.
    // Without this, `const emit = defineEmits([...])` captured the raw
    // ARRAY, so every script-level emit('...') threw "emit is not a
    // function" — which silently broke item picks, dialog Cancel
    // buttons, and close events across 18 components. The comma
    // operator turns `(__emits_decl = [...])` into
    // `(__emits_decl = [...], __ctx_emit)`: the declaration is still
    // harvested statically, and the expression now yields the real
    // emit function (resp. reactive props object).
    body = _appendAtDeclClose(body, '(__props_decl = ', ', __ctx_props');
    body = _appendAtDeclClose(body, '(__emits_decl = ', ', __ctx_emit');
  ...
  // Insert `suffix` immediately before the close-paren that matches the
  // `(` at the end of `marker`, for every occurrence. String-aware,
  // depth-tracked — the same scanning rules as extractDecl below.
  function _appendAtDeclClose(src, marker, suffix) {
    let out = src;
    let from = 0;
    for (;;) {
      ...
      for (; i < out.length; i++) {
        ...
        if (c === ')' || c === ']' || c === '}') {
          if (depth === 0 && c === ')') {
            out = out.slice(0, i) + suffix + out.slice(i);
            i += suffix.length;
            break;
          }
          depth--;
  ...
  function harvestPropsEmits(body, filename) {
    ...
    //
    // Extraction is a balanced-delimiter scan, NOT a lazy regex: props
    // literals routinely contain `)` inside them — e.g.
    // `default: () => []` — and `([\s\S]+?)\)` truncates at the first
    // one (learned the hard way from ModifierPicker.vue).
    const result = { props: null, emits: null };

    function extractDecl(marker) {
      ...
      const start = i;
      let depth = 0;
      let inStr = null;   // ', ", or ` while inside a string
      for (; i < body.length; i++) {
        const c = body[i];
        if (inStr) {
          if (c === '\\') { i++; continue; }
        ...
        if (c === '(' || c === '[' || c === '{') { depth++; continue; }
        if (c === ',' && depth === 0) {
          // The `, __ctx_props` / `, __ctx_emit` tail added by pass 3 —
          // not part of the static literal.
          return body.slice(start, i).trim();
        }
        if (c === ')' || c === ']' || c === '}') {
          if (depth === 0 && c === ')') {
            // The unmatched ')' closing the original defineX( call.
            return body.slice(start, i).trim();
          }
          depth--;
        }
    ...
    const propsSrc = extractDecl('__props_decl');
    if (propsSrc !== null) {
      try { result.props = (new Function(`return ${propsSrc};`))(); }
      catch (e) {
        throw new Error(`SFC ${filename}: defineProps() argument is not a static literal`);
      }
    }
    const emitsSrc = extractDecl('__emits_decl');
    if (emitsSrc !== null) {
      try { result.emits = (new Function(`return ${emitsSrc};`))(); }
      catch (e) {
  ...
  async function fetchSource(path) {
    ...
    // ALPHAX_SPA_FETCH (set by the desk page loader) tries /assets first
    // and falls back to the API supply line when the asset pipeline is
    // broken. Direct fetch remains for standalone/test usage.
    const p = (global.ALPHAX_SPA_FETCH
      ? global.ALPHAX_SPA_FETCH(`cashier/sfc/${path}`)
      : fetch(`/assets/alphax_pos_suite/dist/vendor/cashier/sfc/${path}` + (global.ALPHAX_ASSET_VER ? `?v=${global.ALPHAX_ASSET_VER}` : '')).then(async (r) => {
          if (!r.ok) throw new Error(`Could not fetch SFC ${path}: HTTP ${r.status}`);
          return r.text();
        })
    ).then((text) => {
