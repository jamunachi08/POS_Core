...
(async function () {
  ...
  async function loadESMAsObject(path) {
    // Same supply lines as the SFC loader: /assets first, API fallback
    // when the asset pipeline is broken (see page loader).
    let source;
    if (window.ALPHAX_SPA_FETCH) {
      source = await window.ALPHAX_SPA_FETCH(`cashier/sfc/${path}`);
    } else {
      const url = `${SPA_BASE}/${path}` + (window.ALPHAX_ASSET_VER ? `?v=${window.ALPHAX_ASSET_VER}` : '');
    ...
    // Strip ESM `export` syntax, capturing every exported binding onto a
    // local __exports object.
    //
    // All patterns are anchored to the start of a line (top-level exports
    // are never indented in this codebase) so the word "export" inside a
    // string or a comment is left alone.
    //
    // Covered forms:
    //   export const|let|var X = ...
    //   export function X    /  export function* X
    //   export async function X  /  export async function* X   <-- v15.10.4
    //   export class X
    //   export default ...
    //   export { a, b as c }
    let body = rewritten;
    ...
    // `export { a, b as c }` (possibly spanning lines). Re-export form
    // (`export { x } from './y'`) is intentionally not matched — it is
    // caught by the leftover check below with a readable message.
    body = body.replace(/^[ \t]*export\s*\{([\s\S]*?)\}[ \t]*;?[ \t]*(?=\r?\n|$)/gm,
      function (full, list) {
        return list.split(',').map(function (part) {
          const bits = part.trim().split(/\s+as\s+/);
          const local = (bits[0] || '').trim();
          if (!local) return '';
          const exposed = (bits[1] || bits[0]).trim();
          exportedNames.push(exposed);
          return `__exports.${exposed} = ${local};`;
        }).filter(Boolean).join(' ');
      });

    // `export default ...`
    body = body.replace(/^[ \t]*export\s+default\s+/gm, '__exports.default = ');

    // `export const|let|var X = ...` — assigned inline so the module's own
    // reference semantics (TDZ, ordering) are preserved.
    body = body.replace(/^[ \t]*export\s+(const|let|var)\s+([A-Za-z_$][A-Za-z0-9_$]*)\s*=/gm,
      function (full, kind, name) {
        exportedNames.push(name);
        return `${kind} ${name} = __exports.${name} =`;
      });

    // `export [async] function[*] X` and `export class X` — keyword stripped,
    // binding re-attached in the tail (declarations are hoisted, classes are
    // initialised by the time the tail runs).
    body = body.replace(
      /^[ \t]*export\s+(async[ \t]+function[ \t*]*|function[ \t*]*|class[ \t]+)([A-Za-z_$][A-Za-z0-9_$]*)/gm,
      function (full, kind, name) {
        exportedNames.push(name);
        return `${kind}${name}`;
      });

    // Anything still starting with `export` (or a missed `import`) would
    // blow up inside new Function() with a bare "Unexpected token" and no
    // line context. Fail loudly instead.
    const leftover = body.match(/^[ \t]*(export|import)\s.*$/m);
    if (leftover) {
      throw new Error(
        `Could not evaluate ${path}: unsupported module syntax -> ${leftover[0].trim().slice(0, 160)}`
      );
    }

    // Also re-attach top-level declarations that were never exported. This
    // is historical loader behaviour (some modules rely on cross-module
    // access without an explicit export) and is kept deliberately.
    const declRe = /^(?:async[ \t]+)?(?:function[ \t*]*|class[ \t]+)([A-Za-z_$][A-Za-z0-9_$]*)/gm;
    const names = exportedNames.slice();
    let mm;
    while ((mm = declRe.exec(body))) names.push(mm[1]);

    const seen = Object.create(null);
    const tailNames = names.filter(function (n) {
      if (seen[n]) return false;
      seen[n] = true;
      return true;
    });
    const tail = tailNames.length
      ? '\n' + tailNames.map(n => `try { __exports.${n} = ${n}; } catch (_e) {}`).join('\n')
      : '';
  ...
  window.AlphaXApi = {};
  // mock MUST precede client: client.js does an eager module-scope
  ...
  // -------------------------------------------------------------------
  // Phase B: Stores namespace
  // -------------------------------------------------------------------

  // MUST come before composables: the import rewriter turns
  // `import { usePOSStore } from '../stores/pos'` into an EAGER
  // module-scope destructure of window.AlphaXStores, and
  // composables/useMoney.js does exactly that. Stores themselves only
  // import from the api namespace (Phase A), so this order is safe.
  //
  ...
  // -------------------------------------------------------------------
  // Phase C: Composables namespace
  // -------------------------------------------------------------------

  // haptics first (useLongPress imports it). useBarcodeScanner is
  ...
  const COMPONENTS_MID = [
    ...
    'components/InvoiceHistory.vue',
    'components/ComboPicker.vue',
    'components/DayCloseDialog.vue',
    'components/SplitBill.vue',
    'components/LoyaltyScan.vue',
    'components/RxCapture.vue',
    'components/BootScreen.vue',
    'components/ManagerPinDialog.vue',
    'components/ReturnDialog.vue',
  ...
  app.use(pinia);
  // Support/debug handle: lets on-site diagnostics (and the release
  // verification sim) inspect store state without a dev build.
  window.__ALPHAX_PINIA__ = pinia;
  window.__ALPHAX_APP__ = app;
  app.config.errorHandler = (err, inst, info) => {
    console.error('[alphax] app error (' + info + '): ' + (err && err.stack ? err.stack.split('\n').slice(0,3).join(' | ') : err));
  };
  app.use(i18n);
