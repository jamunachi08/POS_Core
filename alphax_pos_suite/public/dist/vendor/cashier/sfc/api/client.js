...
// Object-method `call` below shadows nothing, but aliasing makes the
// delegation unambiguous to a reader.
const invoke = call

export const api = {

  // Generic passthrough for endpoints that do not warrant a named wrapper
  // (onboarding wizard, top movers). Without it `api.call is not a
  // function` kills any caller that reaches for a raw method path.
  //
  // Mock mode: delegate to the mock provider when it knows the method,
  // otherwise resolve null so callers fall back to their empty state
  // instead of throwing inside a wizard with no way out.
  call(method, args = {}, opts = {}) {
    if (useMock()) {
      if (typeof mock.call === 'function') return mock.call(method, args)
      return Promise.resolve(null)
    }
    return invoke(method, args, opts)
  },
  ...
  // v15.12 — the server decides the sellable range. The old listItems()
  // below asked frappe.client.get_list for Items directly, which meant
  // the browser chose what it was allowed to see and capped silently at
  // 200 rows. Kept only as the fallback for a server without the new
  // endpoint.
  menuItems({ terminal = null, outlet = null, search = null, limit = 500, start = 0 } = {}) {
    if (useMock()) return mock.listItems().then(items => ({ items, total: items.length, source: 'mock' }))
    return call('alphax_pos_suite.alphax_pos_suite.catalog.api.list_menu_items',
                { terminal, outlet, search, limit, start })
  },

  sessionScope() {
    if (useMock()) return Promise.resolve({ mode: 'open', branches: [] })
    return call('alphax_pos_suite.alphax_pos_suite.session.api.get_session_scope')
  },

  chooseBranch(branch, terminal) {
    if (useMock()) return Promise.resolve({ branch, terminal, needs_terminal: false })
    return call('alphax_pos_suite.alphax_pos_suite.session.api.choose_branch',
                { branch, terminal })
  },
  ...
  tableAdminState(outlet) {
    if (useMock()) return Promise.resolve({ can_manage: false, floors: [], suggested_code: 'T1' })
    return call('alphax_pos_suite.alphax_pos_suite.floor.api.table_admin_state', { outlet })
  },

  quickAddTable(table_code, seats, floor, outlet) {
    if (useMock()) return Promise.resolve({ ok: true, table: { table_code, seats, status: 'Free' } })
    return call('alphax_pos_suite.alphax_pos_suite.floor.api.quick_add_table',
                { table_code, seats, floor, outlet })
  },
