...
export const mock = {
  ...
  // Generic passthrough used by api.call() in mock mode. The onboarding
  // wizard runs before anything else, so a demo (`?mock=1`) with no answers
  // here would stall on step 1 forever. Report an already-bound station and
  // let the wizard walk straight through to the till.
  async call(method, args = {}) {
    ...
    if (m === 'probe') {
      return {
        decision: 'bound',
        can_provision: false,
        fingerprint: 'mockfingerprint000000000000',
        server: { client_ip: '127.0.0.1' },
        terminal: { name: 'TERM-DEMO', pos_outlet: 'Demo Outlet', pos_profile: 'Demo Profile' },
        candidates: [],
      }
    }
    if (m === 'get_bridge_installers') return { methods: [], recommended: null }
    if (m === 'bridge_status') return { online: false }
    if (m === 'claim' || m === 'auto_provision') {
      return { terminal: { name: 'TERM-DEMO', pos_outlet: 'Demo Outlet', pos_profile: 'Demo Profile' } }
    }
    return null
  },
