...
export const useHardwareStore = defineStore('hardware', () => {
  ...
  async function printKot(bridgeTarget, ticket) {
    // Station fan-out: each Print Station names its bridge device; the
    // one bridge in the shop relays ESC/POS to that printer's LAN IP.
    if (!online.value) return { ok: false, reason: 'bridge offline' }
    if (!bridgeTarget) return { ok: false, reason: 'station has no bridge device name' }
    try {
      await bridge.print(bridgeTarget, ticket)
      return { ok: true, device: bridgeTarget }
    } catch (e) {
  ...
  return {
    ...
    printReceipt, printKot, kickDrawer, showOnDisplay, readWeight,
