<script setup>
// (computed added v15.7.4 — see paymentMethods below)
...
// The register must never present ZERO tender options. Server-side
// pos_boot now falls back to enabled Modes of Payment; this guards the
// window where an old cached boot payload (empty list) is still live.
const paymentMethods = computed(() => {
  const fromBoot = store.boot?.payment_methods || []
  if (fromBoot.length) return fromBoot
  const cash = store.settings.cash_mode_of_payment || 'Cash'
  return [{ mode_of_payment: cash, type: 'Cash', is_default: 1 }]
})
  ...
  try {
    ...
    if (hw.online) {
      try {
        ...
        // KOT fan-out at sale time — one ticket per kitchen station,
        // straight through the bridge, online or offline. The server
        // creates the matching KDS tickets on submit.
        try {
          const kotCfg = store.boot?.kot || {}
          if (kotCfg.mode !== 'No Kitchen Tickets') {
            const copies = Math.max(1, Number(kotCfg.copies) || 1)
            for (const grp of store.routeCartToStations()) {
              // A merged single-printer group carries no station_type —
              // it is a synthesised target, not a configured station.
              if (!grp.merged && grp.station.station_type !== 'Printer') continue
              const ticket = store.buildKotTicket(grp, saleName)
              for (let i = 0; i < copies; i++) {
                hw.printKot(grp.station.bridge_target, ticket).catch(() => {})
              }
            }
          }
        } catch { /* kitchen paper must never block the sale */ }
