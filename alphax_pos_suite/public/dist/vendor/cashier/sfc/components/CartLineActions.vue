...
const emit = defineEmits(['close', 'edit-modifiers', 'request-approval'])
...
function applyDiscount() {
  ...
  const doApply = () => {
    const newRate = +(props.line.rate * (1 - clamped / 100)).toFixed(4)
    store.setRate(props.line.line_uuid, newRate)
    haptics.success()
  }

  // Policy gate: discounts above the configured threshold need a
  // manager PIN. The parent owns the PIN dialog; we hand it a
  // continuation to run once authorized.
  const s = store.settings || {}
  const threshold = Number(s.discount_threshold_percent) || 0
  if (s.require_manager_for_discount && clamped > threshold) {
    emit('request-approval', { action: `Discount ${clamped}%`, run: doApply })
    emit('close')
    return
  }
  doApply()
...
function voidLine() {
  ...
  // Policy gate: voiding a line needs a manager PIN when configured.
  if (store.settings?.require_manager_for_void) {
    emit('request-approval', { action: 'Void Line', run: doVoid })
    emit('close')
    return
  }
