...
onMounted(async () => {
  loading.value = true
  try {
    terminals.value = await api.listTerminals() || []
    // One terminal and nothing stored: this IS the terminal — don't
    // make the cashier tap a one-item list on every fresh device.
    if (terminals.value.length === 1) pick(terminals.value[0].name)
  } catch (e) {
    // v15.7.5 swallowed this into "No terminals configured", hiding
    // permission problems. Show the server's message verbatim.
  ...
  <div class="boot-screen">
    ...
    <div class="boot-card">
      ...
      <div v-else class="terminal-picker">
        ...
        <div v-else-if="listError" class="boot-error">
          <div class="error-title">{{ t('errors.terminal_list_failed') }}</div>
          <div class="error-detail">{{ listError }}</div>
        </div>
        <div v-else-if="terminals.length === 0" class="muted">{{ t('errors.no_terminals') }}</div>
        <div v-else class="terminal-list">
          <button
            v-for="term in terminals"
            :key="term.name"
            class="terminal"
            @click="pick(term.name)"
          >
            <div class="t-mark">⌥</div>
            <div class="t-body">
              <div class="t-name">{{ term.name }}</div>
              <div class="t-sub" v-if="term.pos_outlet || term.branch">
                {{ [term.pos_outlet, term.branch].filter(Boolean).join(' · ') }}
              </div>
            </div>
          </button>
        </div>
...
.t-body { min-width: 0; text-align: start; }
.t-name { font-weight: 600; font-size: 14px; }
.t-sub { color: var(--text-dim); font-size: 12px; margin-block-start: 2px; }
