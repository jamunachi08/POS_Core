...
// The table chip follows the active service mode, not just the venue.
// Takeaway and delivery never seat anyone, so asking for a table there is
// noise at best and a wrong KOT route at worst.
const showTable    = computed(() => store.tableApplies)
const tableMissing = computed(() => store.tableRequired && !store.activeTable)
...
<template>
  <div v-if="anyVisible" class="ribbon">
    <button v-if="showTable" class="chip"
            :class="{ filled: store.activeTable, required: tableMissing }"
            @click="emit('pick-table')">
      <span class="chip-icon">⬚</span>
      <span class="chip-label">
        <span class="chip-title">
          {{ t('context.table_chip') }}
          <span v-if="tableMissing" class="req-star">*</span>
        </span>
        <span class="chip-value">
          {{ store.activeTable
             || (store.tableRequired ? t('context.table_required') : t('context.pick_table_short')) }}
        </span>
      </span>
    </button>
...
.chip.required .chip-value { color: var(--danger, #C0392B); font-weight: 700; }
.req-star { color: var(--danger, #C0392B); }
