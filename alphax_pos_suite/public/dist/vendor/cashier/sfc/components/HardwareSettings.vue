...
// "Whenever they want, they choose and install." Reopening the picker
// drops the terminal back into the setup wizard at the hardware step —
// same screen as first-run, so there is nothing new to learn.
const planBusy = ref(false)
async function changeHardware() {
  planBusy.value = true
  try {
    if (!ob.terminal?.name && pos.terminal) ob.terminal = { name: pos.terminal }
    await ob.reopenHardware()
    // Dropping the boot payload sends App.vue back to the wizard, which
    // is already sitting on the hardware step.
    pos.boot = null
    emit('close')
  } finally {
...
<template>
  <AppModal :title="t('hardware.settings')" size="lg" @close="emit('close')">
    ...
    <!-- what this station is meant to have -->
    <section class="section">
      <div class="section-head">
        <h4>{{ t('hardware.plan') }}</h4>
        <button class="btn ghost sm" :disabled="planBusy" @click="changeHardware">
          {{ t('hardware.change_plan') }}
        </button>
      </div>
      <p class="hint">{{ t('hardware.plan_hint') }}</p>
    </section>
