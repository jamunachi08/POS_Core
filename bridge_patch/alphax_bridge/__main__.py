...
def main(argv=None):
    ...
    # Printers added from a network scan live in their own JSON file so the
    # hand-edited config is never rewritten. Config entries win on a clash.
    from .netdevices import load_into as _load_net
    _n = _load_net(registry)
    if _n:
        logging.getLogger("alphax-bridge").info("loaded %d scan-added network device(s)", _n)
