...
Endpoints:
  ...
  GET  /hello             — PUBLIC. name, version, hostname only. Lets another
                            bridge on the LAN recognise this one during a scan.
  GET  /discover          — auto-detect USB / serial candidates on this PC
  GET  /discover/network  — ?target=<ip | cidr up to /24 | empty for local /24>
                            find printers and other bridges on the LAN
  POST /devices/network   — { "name", "host", "port", "profile"? }
                            register a discovered network printer, persisted
...
def make_app(registry: DeviceRegistry, auth_token: Optional[str] = None,
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _set_cors(self):
            ...
            # Chromium's Private/Local Network Access: an HTTPS page (the POS
            # on Frappe Cloud) calling localhost sends a preflight asking for
            # this. Without it the browser can block the call outright.
        ...
        def do_GET(self):
            ...
            if url.path == "/hello":
                # Deliberately unauthenticated and deliberately thin: enough for
                # a neighbouring bridge to say "that address is till 3", nothing
                # an unauthenticated caller could use. No UUID, MAC or devices.
                return self._json(HTTPStatus.OK, {
                    "name": "alphax-pos-bridge",
                    "version": __version__,
                    "hostname": (system_info() or {}).get("hostname", ""),
                })
            ...
            try:
                ...
                if url.path == "/discover/network":
                    target = (qs.get("target") or [""])[0]
                    try:
                        return self._json(HTTPStatus.OK, netscan.scan(target))
                    except netscan.ScanError as e:
        ...
        def do_POST(self):
            ...
            try:
                if url.path == "/devices/network":
                    return self._handle_add_network_device(body)
                if url.path == "/print":
        ...
        def _handle_add_network_device(self, body):
            from .netdevices import add_network_device
            try:
                return self._json(HTTPStatus.OK, add_network_device(registry, body))
